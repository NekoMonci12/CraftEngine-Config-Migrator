...
/**
 * Logger for ItemsAdder assets copy
 */
function loggerItemsAdderAssets(level, message) {
  log(message, level, 'itemsadderassets')
}
...
function copyFolderRecursive(src, dest, blacklist = [], whitelist = []) {
  if (!fs.existsSync(src)) {
    loggerItemsAdderAssets('warn', `Source folder not found: ${src}`)
    return
  }
  ...
  for (const entry of entries) {
    ...
    if (blacklist.some(bl => relativeSrc.startsWith(bl))) {
      loggerItemsAdderAssets('warn', `Skipped blacklisted path: ${srcPath}`)
      continue
    }
    ...
    if (entry.isDirectory()) {
      ...
      if (!whitelist.includes(ext)) {
        loggerItemsAdderAssets('warn', `Skipped non-whitelisted file: ${srcPath}`)
        continue
      }

      fs.copyFileSync(srcPath, destPath)
      loggerItemsAdderAssets('info', `Copied file: ${srcPath} -> ${destPath}`)
    }
  }
}
...
function generateTextureAtlas(outputFolder) {
  ...
  if (!fs.existsSync(texturesFolder)) {
    loggerItemsAdderAssets('warn', `Textures folder not found: ${texturesFolder}`)
    return
  }
  ...
  loggerItemsAdderAssets('info', `Generated texture atlas at: ${atlasFile}`)
}

/**
 * Handle ItemsAdder resource pack assets
 * Copies input/resource_pack/assets -> output/resourcepack/assets
...
function convertAssets(inputFolder, outputFolder) {
  // ItemsAdder can have both resource_pack and resourcepack folders
  let srcFolder = path.join(inputFolder, 'resource_pack', 'assets')
  if (!fs.existsSync(srcFolder)) {
    srcFolder = path.join(inputFolder, 'resourcepack', 'assets')
  }
  ...
  loggerItemsAdderAssets('info', `Resource pack assets copied to: ${destFolder}`)
